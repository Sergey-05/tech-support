...
export async function POST(
  ...
  try {
    ...
    const preparedFiles = await Promise.all(
      files.map(async (file) => {
        if (file) {
          ...
          return {
            request_id: id,
            attachment_uploaded_by: user_id, // TODO: Replace with actual user ID
    ...
    // Загрузка файлов в хранилище
    for (const file of preparedFiles) {
      const { data, error: uploadError } = await storage.upload(file.attachment_path, file.attachment_name);
